...
public class UserInterface extends JPanel {
	...
	public UserInterface(int size, int row, int col){
		rows = row;
		cols = col;		
	...
	private void drawState(Graphics g){
		int numMines = GraphicsController.grid.size()/8; 
		Integer rem = numMines - GraphicsController.mine.size();
	...
	private void drawGrid(Graphics g){
		...
		for (int y = startHeight; y < windowHeight + startHeight; y += windowHeight/rows){
			for (int x = 0; x < windowWidth; x += windowWidth/cols){								
				...
				int score = model.Board.getNode(count).score();
				...
				g.setFont(f);
				if (score > 0 && !model.Board.getNode(count).isMine()) g.drawString(score + "", x + (int) (windowWidth/(2.5*cols)), y + (int) (windowHeight/(1.5*rows)));
				g.setColor(Color.BLACK);
				if (model.Board.getNode(count).isMine()) g.drawString("X", x + (int) (windowWidth/(3*cols)), y + (int) (windowHeight/(1.5*rows)));
				count++;
			}
		}
	}
